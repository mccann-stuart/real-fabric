...
As reconciled on 17 September 2026 with the current repository state:

- The React/Vite client, SQLite Durable Object room service, control-plane WebSocket, presenter simulation, browser media components, inspector, telemetry and failure registry are implemented with 439 automated tests across 27 files.
- Recent security review hardening is implemented and verified: presenter/creator authorization prevents unauthorized AI lifecycle and presenter operations (SEC-02), public snapshots omit detailed routing choices while authenticated snapshots expose only the viewer's rows and retain anonymous partial-context state (SEC-04), control-plane WebSocket credentials moved from query string to in-message authentication (SEC-05), one active control socket per participant enforced in the Durable Object (SEC-06), client-IP room join rate limiting enforced (SEC-07), streaming request body size capped at 32 KiB (SEC-08), playback deduplication bounded at 100 objects per group (SEC-09), receive-side media bursts bounded at the jitter buffer, drain tick and decoder queue (SEC-10), deferred DDL schema migrations prevent unknown room probes from allocating persistent SQLite Durable Object state (SEC-11), and explicit capture capability checks prevent narrow or unsupported read-only clients from initiating microphone capture (SEC-12).
...
- `wrangler.jsonc` pins MOQT draft 16, configures the Cloudflare isolated relay URL, and sets `MOQT_TRANSPORT_VERIFIED=true` following Gate 1 acceptance testing, with `MOQ_ROUTING_ENFORCEMENT=cooperative` and `MOQ_DISCOVERY=unknown`.
- `moqtail@0.12.1` frames draft 16. `MoqTransportAdapter` attempts draft-16 transport with provisioned token in URL path, uses the relay-supported `PUBLISH` request directly, and answers permitted room-namespace pushes with `PUBLISH_OK` before routing their object streams through the ordinary subscription path. A narrow pnpm patch preserves the caught MOQtail control-stream termination cause. Live transport is trace-verified against `draft-16.cloudflare.mediaoverquic.com` (`MOQT_TRANSPORT_VERIFIED=true`, evidence in `reports/gate1-transport-trace.json` and NetLog `reports/gate1-transport.netlog`).
- Provisioned relay-token handling is implemented, but it has a known P1: unauthenticated room creation and open room joining return any locally current configured `MOQ_RELAY_TOKEN` to the browser. The credential grants publish and subscribe across the whole relay and can be reused outside the room service until expiry or revocation.
- The Worker rejects malformed relay JWTs and JWTs whose `exp` claim has passed before returning them to a browser, and `/api/health` exposes only the resulting non-secret status. This is a fail-closed availability check, not signature verification or a fix for the relay-wide credential scope. The production token observed on 9 September 2026 remains expired until an explicitly authorised secret rotation.
...
- Dynamic device tracking, packet-loss concealment, bounded recovery and silence-gated drift correction are implemented and unit-tested, but lack live acoustic acceptance. Receive-side capacity accounting now uses per-window underrun deltas only for active tracks, excluding prebuffering, explicit DTX silence and inactive tracks; a one-remote resubscription regression covers the former eight-second capacity loop. Drift estimation uses a robust long window against the AudioContext output clock, resets on discontinuities and resubscription, applies correction in the worklet's actual consumption direction, rate-limits drift events, and clears an uncorrectable-drift warning only after a buffer rebuild and sustained convergence.
...
- Stored-session handling is keyed on the normalised room code across store, load and clear, so the three helpers cannot disagree about an entry. `loadSession` enforces `REJOIN_WINDOW_MS` client-side and fails closed: a session past the window, or one carrying no usable `storedAt`, is refused rather than reloaded into a rejoin the room service would reject. The room service remains authoritative for reclaim.
- The sanitised telemetry export admits an allow-list of fields (`at`, `type`, `participantId`, `trackId`, `value`) rather than excluding named ones, and filters again on export. Free-text values are refused, so a nested or unlisted payload cannot carry a display name, device label or transcript into a report (AC-14).
- Aggregated receive-side counters distinguish absence from zero: with no track subscribed the count is a true `0`, but with tracks subscribed and none of them reporting, `lateDrops`, `cancelledDrops`, `concealedFrames` and `comfortNoiseFrames` read `Not exposed` rather than summing unobserved values to a confident `0` (H15).
...
- H3 now requires a documented matrix of all supported browser, OS and major-version combinations. The client recognises provisional Chrome 141+ on macOS, top-level Safari 27+ on macOS, top-level Safari 27+ on the iOS 27 iPhone target and top-level Chrome for iOS 141+ on that target; all four still require their applicable real-browser acceptance evidence. Safari deliberately freezes the iPhone OS user-agent token at an iOS 18 value, so the client uses `Version/` or `CriOS/` only to identify the admitted top-level browser and then requires concrete secure-context, WebTransport, Opus encoder/decoder, AudioWorklet capture and playout probes. The frozen OS token is informational and never vetoes passing capability evidence. The macOS Safari pin likewise names a browser major only because the `Mac OS X 10_15_7` token is frozen; iPadOS desktop mode is separated from macOS by `navigator.maxTouchPoints` and stays read-only. Firefox, Edge and Opera on iOS, embedded web views and installed Home Screen mode remain read-only.
- Unit tests do not satisfy the acoustic latency, measured-capacity, audible ten-minute or two-clean-run acceptance gates; Gate 1 transport acceptance is satisfied by the live browser-to-relay packet and frame trace.
...
1. **Gate 1 transport acceptance (Completed 10 September 2026):** Passed with reproducible browser-to-relay packet and frame trace over WebTransport and HTTP/3/QUIC proving live MOQT negotiation, publication and subscription with 0.0% loss (`reports/gate1-transport-trace.json` and `reports/gate1-transport.netlog`), with `MOQT_TRANSPORT_VERIFIED=true`.
