...
4. **Live transport trace acceptance (Gate 1 exit — Completed 10 September 2026):** Captured end-to-end browser-to-relay packet and frame trace (`reports/gate1-transport.netlog` and `reports/gate1-transport-trace.json`) over WebTransport and HTTP/3/QUIC proving live negotiation, publication, subscription, and 0.0% loss frame exchange against the Cloudflare isolated relay, setting `MOQT_TRANSPORT_VERIFIED=true`.
