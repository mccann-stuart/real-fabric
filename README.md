...
| H3 — supported browser matrix, others warned | [`pinnedConfiguration.ts`](src/shared/pinnedConfiguration.ts), [`PinnedConfigBanner`](src/client/components/PinnedConfigBanner.tsx), [`UniversalAudioCaptureAdapter`](src/client/audio/UniversalAudioCaptureAdapter.ts) | Configuration detection, capture-path selection and exact frame assembly are unit-tested. Provisional Chrome 141+ on macOS, top-level Safari 27+ on macOS, top-level Safari 27+ on iPhone with iOS 27+ and top-level Chrome for iOS 141+ on iPhone with iOS 27+ are recognised; real-browser acceptance is still outstanding. |
...
The admitted candidates are **Google Chrome 141 or later on macOS**, **top-level Safari 27 or later on macOS**, **top-level Safari 27 or later on iPhone with iOS 27 or later** and **top-level Chrome for iOS 141 or later on iPhone with iOS 27 or later**. Safari 27.x and iOS 27 are currently beta releases and form the initial Safari acceptance target; later Safari/iOS majors may run capability pre-flight but remain explicitly unverified. The macOS Safari pin names a browser major only, because Safari freezes its `Mac OS X 10_15_7` token and exposes no readable OS major. Chrome for iOS is a WebKit shell rather than Blink, so its `CriOS` major names only the shell and the iOS major carries the capability floor. Chromium browsers are never admitted as Safari despite carrying a `Safari/` build token, and iPadOS Safari in desktop mode — which reports the same Macintosh token as a Mac, distinguishable only by `navigator.maxTouchPoints` — stays read-only. Firefox, Edge and Opera on iOS, embedded web views and installed Home Screen mode remain read-only.
