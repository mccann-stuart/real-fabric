...
**Date:** 25 August 2026
**Implementation reconciliation:** 17 September 2026, current repository state
...
| Security hardening landed (SEC-02, SEC-04–SEC-12) | Presenter-only authority, viewer-specific routing projection, anonymous partial-context state, in-message WebSocket auth, 1 active socket/participant, IP join rate limit, 32 KiB body stream limit, deduplication and media-burst bounds, deferred DO SQLite schema migration, read-only capture suppression |
...
- Automated test coverage stands at 439 passing tests across 27 files.
...
- `wrangler.jsonc` pins the operational Cloudflare draft-16 endpoint and sets `MOQT_TRANSPORT_VERIFIED` to `true` following Gate 1 acceptance, with `MOQ_ROUTING_ENFORCEMENT` at `cooperative` and `MOQ_DISCOVERY` at `unknown`. Unknown discovery is probed after live setup and the observed result is recorded in the inspector; it is not inferred from configuration.
...
- The automated suite contains 439 passing tests across 27 files. The Objects and Latency tabs expose session-local object counts, rates, sizes, IDs and arrival ages; local capture and codec callback timings; MOQT request setup; receiver hold; audio output latency; and optional WebTransport RTT statistics where implemented by the browser. Complete figures are compared with specification-defined budgets or targets, while diagnostic-only and partial figures say `Reported · no gate`; acoustic loopback remains `Not exposed`. Test audit fixes (PR #202) unified session storage keying, enforced `loadSession` window deadlines fail-closed, instituted strict telemetry export allow-listing (AC-14), and ensured receive counters distinguish absence from unobserved state (H15). Gate 1 interoperability is accepted with live Chromium NetLog trace evidence (`reports/gate1-transport.netlog` and `reports/gate1-transport-trace.json`). Physical-device Safari 27 acceptance, measured capacity, the audible ten-minute run and two clean venue-network script runs remain open.
