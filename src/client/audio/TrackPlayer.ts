...
/** SEC-10: bound max frames drained per call to prevent main thread/decoder overload. */
export const MAXIMUM_DRAIN_FRAMES_PER_CALL = 25;
/** SEC-10: maximum allowed AudioDecoder backlog before pausing drain. */
  ...
  drain(now: number): void {
    ...
    while (drainedCount < MAXIMUM_DRAIN_FRAMES_PER_CALL) {
      if (
        ...
        (this.decoder.decodeQueueSize ?? 0) >= MAXIMUM_DECODER_QUEUE_SIZE
      ) {
        break;
      }
