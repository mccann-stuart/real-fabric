...
- **Status:** Remediated in codebase (17 September 2026). Floor queue queries (`floorQueue` and `releaseFloorInternal` in `src/worker/room.ts`) join with `participants` table filtering `role = 'ai' AND state != 'left'`, ensuring removed or inactive AIs are omitted from queue snapshots and cannot acquire the floor. Validated by automated tests in `test/room-service.test.ts`.
...
- **SEC-03:** Filtered floor queue queries (`floorQueue` and `releaseFloorInternal` in `src/worker/room.ts`) to active AI participants (`role = 'ai' AND state != 'left'`), preventing removed or inactive AIs from wedging or acquiring the floor upon release.
